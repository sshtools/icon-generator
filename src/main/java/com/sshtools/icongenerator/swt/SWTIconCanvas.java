...
public class SWTIconCanvas {
	...
	public SWTIconCanvas(IconBuilder builder, Display display) {
		...
		bounds = new Rectangle(0, 0, (int) builder.width(),
				(int) builder.height());
		...
		// Text
		int fontSize = fixedFontSize == -1 ? (int) (Math.min(bounds.width,
				bounds.height) / 2.5) : fixedFontSize;
		if (builder.icon() != null) {
			text = builder.icon().toString();
			FontData iconFont = getIconFont();
			FontData fontData = new FontData(iconFont.name, fontSize,
	...
	public void draw(GC canvas) {
		Rectangle actualBounds = new Rectangle(this.bounds.x, this.bounds.y,
		...
		try {
			...
			case ROUNDED:
				...
				break;
			case ROUND:
				canvas.fillOval(actualBounds.x, actualBounds.y,
			...
			if (border > 0) {
				...
				canvas.setForeground(new Color(display, getDarker(paint
						.getRGB())));
				...
				case ROUNDED:
					...
					break;
				case ROUND:
					canvas.drawOval(actualBounds.x, actualBounds.y,
							actualBounds.width, actualBounds.height);
					break;
				default:
	...
	private static FontData getIconFont() {
		if (iconFont == null) {
			try {
				final URL resource = SWTIconCanvas.class
						.getResource("/fontawesome-webfont.ttf");

				/*
				 * SWT can't always load fonts from the classpath, and provides
				 * no way of using an input stream, so we write it to a
				 * temporary file
				 */
	...
	private static RGB getDarker(RGB color) {
		return new RGB((int) (SHADE_FACTORY * color.red),
				(int) (SHADE_FACTORY * color.green),
